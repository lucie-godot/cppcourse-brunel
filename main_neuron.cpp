...
int main () {
	...
	for (int i = 0; i < 12500; i += 1) {
		...
		for(int con_in = 0; con_in < 250; con_in += 1) {  //con_in is the number of connection with an inhibitory neuron for each neuron
			connections [rand()*(12500-10000) + 10000][i] += 1;  //Connections with inhibitory neurons
		};
	...
	while (t_ <= t_stop_) {
		...
		for (int i = 0; i < 12500; i += 1) {  //Est-ce au'il ne vaudrait pas mieux utiliser la taille, le type size_t ????????
			...
			if (neuron[i].update (nu_ext, nb_connections_ex)){  //If the neuron number i spikes. The membrane potential is updated in all cases.
				neuron[i].spike(t_);
